...
export default function PaywallPage() {
  ...
  // External wallet = wallet not generated by backend (connected via MetaMask/WalletConnect)
  const isExternalWallet = user?.generatedWallet?.isGenerated === false ||
    user?.generatedWallet?.provider === 'external' ||
    (!user?.generatedWallet && !!user?.walletAddress);
  // External wallets need MetaMask (window.ethereum) to sign transactions in the browser
  const canSignExternal = isExternalWallet && !!window.ethereum;
  ...
  const handlePay = () => {
    if (isExternalWallet) {
      if (!canSignExternal) {
        toast.error('Please open this page in a browser with MetaMask to sign transactions.');
        return;
      }
      handleExternalPay();
    } else {
    ...
    <div className="space-y-4">
      ...
      {selectedPlan && (
        <GlassCard className="!border-accent/30">
          ...
          {/* External wallet needs MetaMask */}
          {isExternalWallet && !canSignExternal && (
            <div className="bg-warning/10 rounded-xl p-3 mb-4 flex items-center gap-2">
              <Wallet size={16} className="text-warning" />
              <span className="text-xs text-warning font-medium">
                Install MetaMask to sign transactions with your external wallet
              </span>
            </div>
          )}
          ...
          <PrimaryButton
            ...
            disabled={(!walletAddress && !window.ethereum) || (isExternalWallet && !canSignExternal) || isMinting}
